...
- `KAFKA_AUTHENTICATION`: Authentication schema to use. Currently only Kerberos is supported. Set to 'KERBEROS' for a Kerberos setup. Required for [Kerberos setup](#kerberos-with).

- `KERBEROS_PUBLIC_URL`: Public DNS of the kerberos server to use. Required for [Kerberos setup](#kerberos-with).

- `KERBEROS_PRINCIPAL`: The principal that kafka should use from the kerberos server. Required for [Kerberos setup](#kerberos-with).
