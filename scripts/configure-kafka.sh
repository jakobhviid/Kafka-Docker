...
if [ -z "$KAFKA_BROKER_ID" ]; then
    echo -e "\e[1;32mERROR - Missing essential 'KAFKA_BROKER_ID' \e[0m"
    exit 1
else
...
if [ -z "$KAFKA_ADVERTISED_LISTENERS" ]; then
    echo -e "\e[1;32mERROR - Missing 'KAFKA_ADVERTISED_LISTENERS' \e[0m"
    exit 1
else
...
if [ -z "$KAFKA_LISTENERS" ]; then
    echo -e "\e[1;32mERROR - Missing 'KAFKA_LISTENERS' \e[0m"
    exit 1
else
    # adding a healthcheck port (55555)
...
if ! [ -z ${KAFKA_AUTHENTICATION} ]; then
    ...
    if [[ ${KAFKA_AUTHENTICATION} == KERBEROS ]]; then
        ...
        if [[ -z "${KAFKA_KERBEROS_PUBLIC_URL}" ]]; then
            echo -e "\e[1;32mERROR - Missing 'KAFKA_KERBEROS_PUBLIC_URL' environment variable. This is required to enable kerberos \e[0m"
            exit 1
        fi

        if [[ -z "${KAFKA_KERBEROS_REALM}" ]]; then
            echo -e "\e[1;32mERROR - Missing 'KAFKA_KERBEROS_REALM' environment variable. This is required to enable kerberos \e[0m"
            exit 1
        fi
        ...
        if [[ -z "${KAFKA_KERBEROS_PRINCIPAL}" ]]; then
            if [[ -z "${KAFKA_KERBEROS_API_URL}" ]]; then
                echo -e "\e[1;32mERROR - One of either 'KAFKA_KERBEROS_PRINCIPAL' or 'KAFKA_KERBEROS_API_URL' must be supplied! It is required to enable kerberos for kafka! \e[0m"
                exit 1
                ...
                if [[ -z "${KAFKA_KERBEROS_API_KAFKA_USERNAME}" ]]; then
                    echo -e "\e[1;32mERROR - Missing 'KAFKA_KERBEROS_API_KAFKA_USERNAME' environment variable. This is required to use kerberos API for kafka keytab \e[0m"
                    exit 1
                fi
                if [[ -z "${KAFKA_KERBEROS_API_KAFKA_PASSWORD}" ]]; then
                    echo -e "\e[1;32mERROR - Missing 'KAFKA_KERBEROS_API_KAFKA_PASSWORD' environment variable. This is required to use kerberos API for kafka keytab \e[0m"
                    exit 1
                fi
                ...
                response=$(curl --fail --max-time 5 -X POST -H "Content-Type: application/json" -d "{\"username\":\""$KAFKA_KERBEROS_API_KAFKA_USERNAME"\", \"password\":\""$KAFKA_KERBEROS_API_KAFKA_PASSWORD"\"}" "$KAFKA_KERBEROS_API_URL" -o "$kafka_keytab_location" && echo "INFO - Using the keytab from the API and a principal name of '"$KAFKA_KERBEROS_API_KAFKA_USERNAME"'@'"$KAFKA_KERBEROS_REALM"'" || echo "FAIL" )
        ...
        if [[ -z "${KAFKA_ZOOKEEPER_KERBEROS_PRINCIPAL}" ]]; then
            if [[ -z "${KAFKA_KERBEROS_API_URL}" ]]; then
                echo -e "\e[1;32mERROR - One of either 'KAFKA_ZOOKEEPER_KERBEROS_PRINCIPAL' or 'KAFKA_KERBEROS_API_URL' must be supplied! It is required to enable kerberos for zookeeper! \e[0m"
                exit 1
            else # User wants to use the kerberos API
                if [[ -z "${KAFKA_KERBEROS_API_ZOOKEEPER_USERNAME}" ]]; then
                    echo -e "\e[1;32mERROR - Missing 'KAFKA_KERBEROS_API_ZOOKEEPER_USERNAME' environment variable. This is required to use kerberos API for zookeeper keytab \e[0m"
                    exit 1
                fi
                if [[ -z "${KAFKA_KERBEROS_API_ZOOKEEPER_PASSWORD}" ]]; then
                    echo -e "\e[1;32mERROR - Missing 'KAFKA_KERBEROS_API_ZOOKEEPER_PASSWORD' environment variable. This is required to use kerberos API for zookeeper keytab \e[0m"
                    exit 1
                fi
            ...
            if ! [[ -f "${zookeeper_keytab_location}" ]]; then
                echo -e "\e[1;32mERROR - Missing zookeeper kerberos keytab file '"$zookeeper_keytab_location"'. This is required to enable kerberos authentication with zookeeper when 'KAFKA_ZOOKEEPER_KERBEROS_PRINCIPAL' is provided. Provide it with a docker volume or docker mount \e[0m"
                exit 1
            else
